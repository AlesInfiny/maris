...
クライアントサイドの例外処理方針は、サーバーサイドと同様に正常なフローに復帰できるかどうかで判断します。
...
これらの例外の捕捉方法について、例外が発生しうる箇所については、同期処理は `try-catch` 、非同期処理は `catch` メソッドを利用して例外を捕捉します。予期せぬ例外が発生した場合は、 Vue.js の `app.config.errorHandler` や JavaScript の `window.onerror` といったグローバルエラーハンドリングで例外を捕捉します。
...
エラーと例外
通常のフローに戻せるパターン　400番台
　システムエラーか判断
戻せないパターン　システムエラー　500番台

バックエンドと同様に、通常フローに復帰できるかでエラーと例外を判断する


#### API 通信で発生する例外 {#api-connection-error}

API 通信には Axios を利用していることを前提としています。 Axios では、業務エラーとシステムエラーは例外として扱われます。
...
クライアントサイドの例外処理方針は、ユーザーが自身で対応できるか、という観点が重要になります、
...
また、ユーザーで問題を解決できない場合、開発者や運用者に通知する必要があります。その際の通知方法やロギングについても考慮する必要があります。
...
vueは動いてるエラー
画面が動かなくなるエラー
API 通信で発生するエラー　→　axios によって例外となる
...
コンソールログは、開発環境では詳細なエラーログを出力し、本番環境ではエラーログを出力しないようにします。
開発者が見て判断できるようにしよう
    フィードバック・通知方法の決め方（ユーザー向け・開発者/運用向け）
    何書くか
...
axios では、業務エラーとシステムエラーは例外として扱われます。
