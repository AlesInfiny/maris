...
│  │  ├─ common   ------------ 業務ロジックを持たないコンポーネント（コンポーネントライブラリからimport）
│  │  │  └─ Spinner.vue ------ スピナー、トースト、モーダルなど
│  │  ├─ $domains　------------ 各業務ロジックと密接に関係するコンポーネント
│  │  └─ basket   ------------ 例 買い物かご
│  │     └─ BasketItem.vue
│  ├─ config/ ---------------- （★★★謎の存在）設定ファイルを格納します。
│  ├─ generated/ ------------- 自動生成されたファイルを格納します。
│  ├─ router/ ---------------- ルーティング定義を格納します。
│  │  ├─ $pages　-------------- URL 本位のルート定義
│  │  └─ basket   ------------ 例 /basket
│  ├─ services/ -------------- サービスに関するファイルを格納します。
│  │  ├─ $resources ----------- API リソース本位のルート定義
│  │  └─ basket   ------------ 例 買い物かご API に対するサービス
│  ├─ shared/ ---------------- （★★★まずい存在）アプリに横断的なものすべて 
│  ├─ stores/ ---------------- store に関するファイルを格納します。
│  │  ├─ $features　----------- 機能本位のストア（store はグローバルなので業務を横断する前提）
│  │  └─ authentication   ---- 例 認証関連ストア 
│  │     └─ authentication.ts　
│  ├─ types/  ---------------- （★本当にここ？）型定義
│  ├─ validations ------------ （★★★謎の存在）
│  ├─ views/ ----------------- ルーティングで指定される vue ファイルを格納します
│  │  ├─ $routes　-------------- URL 本位のフォルダー構成
│  │  └─ basket   ------------- 例 /basket
│  │     └─ basketView.vue  --- 各ルート定義に対応するページ
│  ├─ App.vue
│  └─ main.ts
├─ index.html
└─ package.json
...
```text title="src 直下のフォルダー構成" linenums="0"
<project-name>
─ e2e/
│  ├─ page　------------------------ URL 本位のページを表すクラスを配置します。
│  ├─ testHelper/  ----------------- ページを操作するテストヘルパーを配置します。
│  ├─ testScenario/ ---------------- 検証するテストシナリオを配置します。
│     └ BusinessScenario.test.ts --- 検証対象の画面操作を実装します。 mount して 操作
│  ├─ playwright.config.ts ---------
├─ src/
│  ├─ system-common/ ---------- システム共通機能
│  │  ├─ generated/ ----------- 自動生成された　API　クライアント
│  │  ├─ router    ------------ ルーティング定義
│  │  ├─ stores        -------- アプリケーション全体で利用するストア
│  │  ├─ components    -------- 横断して再利用するコンポーネント
│  │  └─ composables   -------- 横断して再利用するコンポーザブル
│  ├─ business-common/ -------- 業務共通機能
│  │  ├─ components    -------- 横断して再利用する業務的な意味を持つコンポーネント
│  │  └─ composables   -------- 横断して再利用する業務的な意味を持つコンポーザブル
│  ├─ pages/           -------- URL 本位の各画面で、ユースケースを呼び出す。
│  ├─ usecases/        -------- ユースケース
│  │  ├─ usecase1/ ------------ ユースケース1
|  │  │  ├─ __tests__/       -- ユースケース1 のユースケーステスト
|  │  │  └─ features/　      -- ユースケース1 を構成する機能
|  │  │    ├─ feature1/      -- ユースケース1 を構成する機能1
|  │  │    │  └─ components/
|  │  │    │    └─ __tests__/-- 
|  │  │    │  └─ composables/-- 機能1を構成するロジック
|  │  │    │    └─ __tests__/--  ロジックに対する単体テスト
|  │  │    └─ feature2/      -- ユースケース1 を構成する機能2
|  │  │       └─ components/
|  │  │         └─ __tests__/-- 
|  │  │       └─ composables/-- 機能2を構成するロジック
|  │  │         └─ __tests__/-- ロジックに対する単体テスト
|  └─ usecase2/           -- ユースケース2
|  └─ finance/           -- ユースケース　財務業務
         /pages          -- 機能1,2,3 を組み合わせたページ　プロジェクトの特性による
         └─ F-1/         -- 機能1 機能
            /stores      -- 各機能でのみ使用する store
            /composables  --
            /components  --
            /pages       -- 本来（構成図によれば）ここだが、運用上別フォルダに切るのはアリ、 Component はsystemcommon buisinesscommon から呼ぶのは OK 他の機能から呼ぶとまずい
         └─ F-2/         -- 機能2
|  └─ S/           -- ユースケース2

|  └─ shopping/           -- ユースケース　の切り方にセンスが試されている　バックエンドの分割と合っているか？　組織的事情により合いそう
      └─ catalog/         -- 機能1 機能
      └─ order/         -- 機能1 機能
      └─ basket/         -- 機能1 機能
         /stores      -- 各機能でのみ使用する store
         ...
         /composables  --
            fetchBasket.ts
            removeItemFromBasket.ts
            updateItemInBasket.ts
         /components  --
            BasketItem.vue
         /pages       
            BasketView.vue
...
この構成が最も図や説明と整合的
   └─ finance/           -- ユースケース　財務業務
      ...
      └─ F-1/            -- 機能1
         ...
         /components     -- UI
            __コンポーネントテスト
         SearchPage.vue  -- 本来（構成図によれば）ここだが、運用上別フォルダに切るのはアリ、 Component はsystemcommon buisinesscommon から呼ぶのは OK 他の機能から呼ぶとまずい
      └─ F-2/            -- 機能2
...
pages をもっとも外側にすると、Component分割しなくなるかも
ただ、再利用性がないものは分割しない
↓その結果
page しかなくて、 feature がないことがありうる

components なし composable あり もありうる 純ロジック機能
components あり composable なし もありうる 純 UIパーツ

ページがあるのでフォルダーも一致するものがなければならないというわけでもない

!!! pages の配置
プロジェクトの特性によって配置しましょう
   ガイドする
...
components フォルダーは主に、再利用性の高い vue コンポーネントファイルを格納します。さらにこの下層フフォルダはドメインで分割し、それを操作するコンポーネントを格納します。こうすることで再利用性を活かすために、どのドメインを対象にしたコンポーネントなのかを明確にします。また vue ファイルに限らずプロジェクト内で再利用性の高いもの（icon など）もこちらに格納します。
