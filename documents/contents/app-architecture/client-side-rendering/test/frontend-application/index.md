...
AlesInfiny Maris では、継続的インテグレーションを目的として、静的解析から E2E テストの一部までを自動化し、品質と開発スピードの両立を目指します。

| テストレベル       | 名称             | 自動／手動 | テストツール                           |
| ------------------ | :--------------- | ---------- | :------------------------------------- |
| 単体テスト ( UT0 ) | 静的解析         | 自動       | リンター、フォーマッター、型チェッカー |
...
たとえば買い物かご機能の機能内結合テストでは、買い物かご内の商品の数量の変更をテストします。
