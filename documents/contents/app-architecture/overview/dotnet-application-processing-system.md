...
- サーバー故障への対応
    ...
    ヘルスチェック機能でサーバーの稼働状況を監視することで、サーバーの異常を検知できます。
    サーバーの異常を検知した際には正常なサーバに自動的に動作を引き継ぐことで、 Web システムの運用停止を防ぎます。

このように、 LB のサーバー監視によって負荷分散を行い、運用停止を防ぐために、ヘルスチェック機能を実装する必要があります。
...
ここで、 Layer 3 や Layer 4 で行うヘルスチェックは、作成した Web アプリケーションの稼働状況まで確認できません。
...
- ステータスコード
    ...
    Azure や AWS といったクラウドサービスにおける HTTP レスポンスのステータスコードは、主に 200 を正常値、 4xx ～ 5xx を異常値としています。
