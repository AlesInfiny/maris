...
Blazor ランタイム内で発生した未処理例外と、 Blazor の起動前に .NET ランタイム内で発生した未処理例外に共通的に対処するために、`ErrorBoundary` コンポーネントとエラーページを追加します。
具体的な実装方法については、 [集約エラーハンドラーの実装] を参照してください。
...
Blazor Web アプリでは、 SignalR によるクライアント / サーバー間の双方向通信が行われます。
開発作業中に意図したとおりの通信が行われているか確認するために、開発環境では SignalR 通信ログを出力するよう設定することを推奨します。
...
```json title="appsettings.json の設定例" hl_lines="4"
{
"Logging": {
    "LogLevel": {
    "Default": "Information",
