---
title: アプリケーション 開発手順 SSR 編
description: AlesInfiny Maris OSS Edition で開発可能な CSR アプリケーションの開発手順を示すガイドラインです。
---

# アプリケーション開発手順 SSR 編 {#top}

AlesInfiny Maris OSS Edition （以降、 AlesInfiny Maris ） で開発可能な SSR （クライアントサイドレンダリング）アプリケーションの開発手順を示すガイドラインです。

- [事前準備](./getting-started/index.md)
- [ソリューション構造の作成](./solution-structure/index.md)
- [プロジェクトの作成](./create-project/index.md)
- [プロジェクトの共通設定](./common-configuration/index.md)
- [開発に使用するパッケージ](./packages-used/index.md)
- [Fluent UI Blazor プロジェクトの構成](./fluentui-blazor-structure/index.md)
- [CSS の設定と CSS フレームワークの適用](./css-setup/index.md)
- [入力値検証](./input-validation/index.md)
- 画面をまたがる情報の保持（後日追加予定）
- 集約エラーハンドラーの実装（後日追加予定）
- 結合テストプロジェクトの構成（後日追加予定）
- [ヘルスチェック API の実装](./health-checks/index.md)
- [ローカル開発環境の構築](./local-development/index.md)
