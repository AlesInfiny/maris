...
public class CatalogApplicationService
{
    ...
    public async Task<(IReadOnlyList<CatalogItem> ItemsOnPage, int TotalItems)> GetCatalogAsync(int skip, int take, long? brandId, long? categoryId, CancellationToken cancellationToken = default)
    {
        this.logger.LogDebug(ApplicationCoreMessages.CatalogApplicationService_GetCatalogStart, brandId, categoryId);
        ...
        var totalItems = await this.catalogRepository.CountAsync(
            item =>
                ...
                (!categoryId.HasValue || item.CatalogCategoryId == categoryId),
            cancellationToken);
        this.logger.LogDebug(ApplicationCoreMessages.CatalogApplicationService_GetCatalogEnd, brandId, categoryId);
