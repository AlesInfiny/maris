...
public class ScopedTest(ITestOutputHelper testOutputHelper) : TestBase(testOutputHelper)
{
    ...
    [Fact]
    public async Task Scopedで登録したインスタンスはコマンドの実行時に破棄されていない()
    {
        // Arrange
        ObjectStateHistory.Clear();
        using var app = this.CreateHost();
    ...
    [Fact]
    public async Task Scopedで登録したインスタンスはホストの終了時に1回破棄が実行される()
    {
        // Arrange
        ObjectStateHistory.Clear();
        using var app = this.CreateHost();
    ...
    [Fact]
    public async Task Scopedで登録したインスタンスはホストの終了時に1回破棄される()
    {
        // Arrange
        ObjectStateHistory.Clear();
        using var app = this.CreateHost();
    ...
    [Fact]
    public async Task Scopedで登録したインスタンスは型ごとに1つだけ()
    {
        // Arrange
        ObjectStateHistory.Clear();
        using var app = this.CreateHost();
